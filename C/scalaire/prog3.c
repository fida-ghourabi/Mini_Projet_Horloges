...
void display_clock(const char *event) {
    printf("[Processus 3] %s | Horloge scalaire : %d\n", event, scalar_clock);
}
...
DWORD WINAPI receive_thread(LPVOID lpParam) {
    ...
    printf("[Processus 3] Serveur pret sur le port %d\n", PORT3);
...
int main() {
    ...
    scalar_clock++;
    display_clock("Evenement local 1 : Message console");
