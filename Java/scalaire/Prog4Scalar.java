...
public class Prog4Scalar {
    static final int MON_ID = 4;
    ...
    static void updateOnReceive(Message msg) {
        ...
        //System.out.printf("[P%d] Recu de P%d | Horloge recue : %d -> Nouvelle horloge : %d%n",MON_ID, msg.senderId, msg.scalarClock, scalarClock);
    ...
    static class Receiver extends Thread {
        public void run() {
            try (ServerSocket serverSocket = new ServerSocket(PORT4)) {
                System.out.printf("[P%d] Serveur en ecoute sur le port %d%n", MON_ID, PORT4);
                while (!stop.get()) {
