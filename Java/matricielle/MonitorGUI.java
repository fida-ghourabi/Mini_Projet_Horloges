...
public class MonitorGUI extends JFrame {
    ...
    public MonitorGUI() {
        setTitle("🕒 Monitor - Horloge Matricielle");
        ...
        new Thread(() -> {
            try (ServerSocket server = new ServerSocket(7003)) {
                System.out.println("🎧 MonitorGUI en écoute sur le port 7003...");
                while (true) {
                    try (Socket client = server.accept();
                         BufferedReader in = new BufferedReader(new InputStreamReader(client.getInputStream()))) {
                            ...
                            // Identifier le processus (par exemple, "[P1]")
                            if (message.startsWith("[P")) {
                                int pid = Integer.parseInt(message.substring(2, 3));
                                logEvent(pid, message);
                            }
